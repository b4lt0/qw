...
WestwoodOWD::WestwoodOWD(QuicConnectionStateBase &conn)
    : quicConnectionState_(conn),
      ...
      lossMaxRtt_(std::chrono::microseconds(200000)) 
      {
...
void WestwoodOWD::onPacketAcked(const CongestionController::AckEvent::AckPacket &packet) {
    ...
    if (delayControl(0.8)) {
        ...
        ssthresh_ = std::max(
            static_cast<uint64_t>(bandwidthEstimate_ * (rttMinUs + (0.3 * (lossMaxRtt_.count() - rttMinUs))) / 1.0e6),
            2 * quicConnectionState_.udpSendPacketLen);
        ...
        owd_ = 0.3 * (lossMaxRtt_.count() - rttMinUs);
        owdv_ = 0;
